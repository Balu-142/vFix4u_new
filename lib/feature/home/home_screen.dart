...
class HomeScreen extends StatefulWidget {
  ...
  const HomeScreen({super.key, this.addressModel, required this.showServiceNotAvailableDialog, this.userInfoModel});
...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  bool _isRetryingCustomerId = false; // Flag to track if we're currently retrying
    ...
    try {
      ...
      if (customerId == null || customerId.isEmpty) {
        if (_customerIdRetryCount < _maxRetries && !_isRetryingCustomerId) {
          ...
          debugPrint('Error: Customer ID is null or empty. Retrying (${_customerIdRetryCount}/$_maxRetries) in 3 seconds...');
          ...
          _hasCheckedRatingDialog = true; // Mark as checked since we're giving up
          return;
        }
      }
      ...
      _hasCheckedRatingDialog = true; // Mark as checked since we caught an exception
    }
  ...
  Future<void> _skipReview(String bookingId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(' Failed to skip review: ${response.statusCode} ${response.body['message'] ?? response.statusText ?? 'Unknown error'}');
      }
    } catch (e, stackTrace) {
      ...
      body: ResponsiveHelper.isDesktop(context)
        ...
        child: RefreshIndicator(
          onRefresh: () async {
            ...
            _hasCheckedRatingDialog = false; // Allow re-check after refresh
